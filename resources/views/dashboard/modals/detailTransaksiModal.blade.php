  ...
  <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-3">
            ...
            <div class="row">
                ...
                <div class="col">
                    <p class="text-end fw-semibold">{{ number_format($totalItem, 0, ',','.')}}</p>
                </div>
            </div>
            <div class="row">
                <div class="col">
            ...
            <div class="row">
                ...
                <div class="col">
                    <p class="text-end fw-semibold">{{ number_format($transaction->gross_amount - ($transaction->application_fee + $transaction->administration_fee), 2, ',','.') }}</p>
                </div>
            </div>
            <div class="row">
                <div class="col">
                ...
                <div class="col">
                    @if ($transaction->owner_response == "Ditolak")
                        <p class="text-end fw-semibold text-danger">Rp0</p>
                    @else
                        <p class="text-end fw-semibold text-success fs-5">{{ number_format($transaction->gross_amount - ($transaction->application_fee + $transaction->administration_fee), 2, ',','.') }}</p>
                    @endif
                </div>
            </div>
        </div>
      ...
      <div class="modal-footer">
        @if ($transaction->owner_response == "Ditolak" || $transaction->owner_response == "Diterima")
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
        @else
            <form action="/karuniamotor/dashboard/transaction-list/order/reject/{{ $transaction->id }}" method="POST">
                @csrf
            ...
            <form action="/karuniamotor/dashboard/transaction-list/order/accept/{{ $transaction->id }}" method="POST">
                @csrf
