  ...
  <div class="modal-dialog modal-xl modal-dialog-centered" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="mb-3">
            ...
            <div class="row">
                ...
                <div class="col">
                    <p class="text-end fw-semibold"><?php echo e(number_format($totalItem, 0, ',','.')); ?></p>
                </div>
            </div>
            <div class="row">
                <div class="col">
            ...
            <div class="row">
                <div class="col">
                    <p class="fw-semibold text-muted">Total Transaksi</p>
                </div>
                <div class="col">
                    <p class="text-end fw-semibold"><?php echo e(number_format($transaction->gross_amount - ($transaction->application_fee + $transaction->administration_fee), 2, ',','.')); ?></p>
                </div>
            </div>
            <div class="row">
                <div class="col">
                    <p class="fw-semibold text-muted">Total Pemasukan</p>
                </div>
                <div class="col">
                    <?php if($transaction->owner_response == "Ditolak"): ?>
                        <p class="text-end fw-semibold text-danger">Rp0</p>
                    <?php else: ?>
                        <p class="text-end fw-semibold text-success fs-5"><?php echo e(number_format($transaction->gross_amount - ($transaction->application_fee + $transaction->administration_fee), 2, ',','.')); ?></p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
      ...
      <div class="modal-footer">
        <?php if($transaction->owner_response == "Ditolak" || $transaction->owner_response == "Diterima"): ?>
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Tutup</button>
        <?php else: ?>
            <form action="/karuniamotor/dashboard/transaction-list/order/reject/<?php echo e($transaction->id); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <button type="submit" class="btn btn-danger" onclick="event.stopPropagation(); return confirm('Anda yakin ingin menolak transaksi ?');">Tolak</button> 
            </form>
            <form action="/karuniamotor/dashboard/transaction-list/order/accept/<?php echo e($transaction->id); ?>" method="POST">
                <?php echo csrf_field(); ?>
                <button type="submit" class="btn btn-primary" onclick="event.stopPropagation(); return confirm('Anda yakin ingin menerima transaksi ?');">Terima</button> 
            </form>
        <?php endif; ?>
      </div>
    </div>
  </div>
</div><?php /**PATH D:\Skripsi\program\karunia-motor-eli\karuniaApp\resources\views/dashboard/modals/detailTransaksiModal.blade.php ENDPATH**/ ?>